...
history_accuracy = []
history_time = []
tps = {key: 0 for key in img_data.mapping.keys()}
fps = {key: 0 for key in img_data.mapping.keys()}
fns = {key: 0 for key in img_data.mapping.keys()}
...
for idx_batch, (imagergb, label_class, labelrgb) in enumerate(img_batch):
    ...
    for key in img_data.mapping.keys():
        tps[key] += torch.sum((pred_class == key) & (label_class == key))
        fps[key] += torch.sum((pred_class == key) & (label_class != key))
        fns[key] += torch.sum((pred_class != key) & (label_class == key))
    ...
    # v_utils.save_image(pred_class.float()/img_data.num_classes, "./eval/gen_image_{}_{}.png".format(0, idx_batch))
    # v_utils.save_image(x.cpu().data, "./eval/original_image_{}_{}.png".format(0, idx_batch))
...
for key in img_data.mapping.keys():
    iou = tps[key] / (tps[key] + fps[key] + fns[key])
    print(key)
    print(iou)
